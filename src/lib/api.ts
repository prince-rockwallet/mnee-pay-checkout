...
// ============================================================================
// Legacy API functions (for backward compatibility with existing merchant SDK)
// ============================================================================
...
/**
 * @deprecated Use createButtonSession instead for hosted buttons
 *
 * Legacy session creation for merchant SDK integration.
 * This calls the merchant server's create-session endpoint.
 */
...
/**
 * @deprecated Not needed for hosted buttons - button config has the price
 *
 * Calculate totals from merchant's backend.
 */
export async function calculateTotals(
  ...
  const response = await fetch(calculateTotalsUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(request),
  });
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({ error: 'Failed to calculate totals' }));
    throw new Error(error.error || error.message || 'Failed to calculate totals');
  }
