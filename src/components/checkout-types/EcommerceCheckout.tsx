...
export function EcommerceCheckout({
  ...
  const { userInfo, setEmail, setShipping, setContact } = useUserInfo();
  ...
  const handleProceed = () => {
    ...
    // Save email, phone and shipping to localStorage for future use
    if (collectEmail && formData.email) {
      setEmail(formData.email);
    }
    if (collectPhone && formData.phone) {
      setContact({ email: formData.email || '', phone: formData.phone });
    }
    if (collectShipping && formData.shipping) {
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setContact({ email: formData.email || '', phone: e.target.value });
                    }}
                    onBlur={handleContactBlur}
